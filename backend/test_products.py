...
for product_data in test_products:
    ...
    print(f"✅ Created product: {data['id']} - {data['name']} (SKU: {data['sku']})")
...
print(f"✅ Correctly rejected duplicate SKU: {response.json()['detail']}")
...
data = response.json()
print(f"✅ Retrieved product: {data['id']} - {data['name']}")
assert data["sku"] == "WIDGET-001", "SKU mismatch"
assert float(data["price"]) == 29.99, "Price mismatch"
...
print(f"✅ Listed {data['total']} products (limit={data['limit']}, offset={data['offset']})")
...
print(f"✅ Pagination works: returned {len(data['items'])} items (offset={data['offset']}, limit={data['limit']})")
...
print(f"✅ Name filter works: found {data['total']} product(s)")
...
print(f"✅ SKU filter works: found {data['total']} product(s)")
...
print(f"✅ Active filter works: found {data['total']} inactive product(s)")
...
assert data["items"][0]["name"] == "Power Tool"
print(f"✅ Description filter works: found {data['total']} product(s)")
...
assert float(data["price"]) == 39.99, "Price should be updated"
assert data["sku"] == "WIDGET-001", "SKU should not change"
print(f"✅ Updated product: {data['id']} - {data['name']} (${data['price']})")
...
print(f"✅ Updated active status: {data['id']} - active={data['active']}")
...
print(f"✅ Deleted product {created_ids[2]}")
...
print(f"✅ Updated product count: {data['total']}")
...
print(f"✅ Combined filters work: found {data['total']} product(s)")

# Test 18: Invalid limit (should use max)
print("\n18. Testing pagination limits...")
response = client.get("/products?limit=200")  # Should be clamped to 100
assert response.status_code == 200
data = response.json()
assert data["limit"] == 100, "Limit should be clamped to 100"
print(f"✅ Limit validation works: {data['limit']}")
...
print(f"✅ Offset validation works")
...
print("\n" + "=" * 80)
print("✅ ALL PRODUCT CRUD TESTS PASSED!")
print("=" * 80)
